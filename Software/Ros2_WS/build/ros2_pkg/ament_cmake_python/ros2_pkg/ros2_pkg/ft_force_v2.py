...
STD_Z, STD_Y, LIFT_H = 0.2, 0.01, 0.07
ACTION_DIM, STATE_DIM, STEPS_PER_PHASE = 4, 7, 10 
...
class PPO:
    ...
    def update(self):
        ...
        print(f"\n\033[1;32m[PPO UPDATE] >>> Training with {min_size} steps...\033[0m")
...
# ==============================================================================
class HumanoidServoNodeV2(Node):
    def __init__(self):
        super().__init__('humanoid_servo_node_v2')
        
        self.base_dir = "/home/du/Desktop/NCKH_2026/Software/Ros2_WS/src/ros2_pkg/weights"
        self.old_force_dir = os.path.join(self.base_dir, "finetunning_force")
        self.force_dir = os.path.join(self.base_dir, "finetunning_force_v2")
        ...
        import shutil
        old_best_model = os.path.join(self.old_force_dir, "best_model.pth")
        old_meta = os.path.join(self.old_force_dir, "training_meta.json")
        if not os.path.exists(self.latest_model_path) and os.path.exists(old_best_model):
            shutil.copy(old_best_model, self.latest_model_path)
            shutil.copy(old_best_model, self.best_model_path)
            if os.path.exists(old_meta):
                shutil.copy(old_meta, self.meta_path)
            print(f"\033[93m>>> Copied best model and meta from finetunning_force to finetunning_force_v2\033[0m")
        ...
        print(f"\033[94m>>> Node Ready. Full Logic Restored (subprocess Push). Initial STD: {self.current_std:.4f}\033[0m")
    ...
    def apply_random_force(self, episode):
        ...
        max_f = min(500.0, 250.0 + (episode // 40) * 10.0)
        threading.Thread(target=self._force_worker, args=(max_f,), daemon=True).start()

    def _force_worker(self, max_f):
        # Áp dụng subprocess để push force như trong reactive_balance.py
    ...
    def check_safety_and_prepare_cmd(self, action):
        ...
        dx_s, dy_s, dz_s, dst_s = smooth_act[0]*0.08, smooth_act[1]*0.03, smooth_act[2]*0.03, smooth_act[3]*0.03
        tz = STD_Z + dz_s
    ...
    def stabilize_robot(self):
        msg = Float64MultiArray(); msg.data = [-0.025, 0.01, STD_Z, 0.0, -0.025, -0.01, STD_Z, 0.0, 1.0]
        for _ in range(3): self.action_pub.publish(msg); self.sim_sleep(0.1)
        self.sim_sleep(1.5)
    ...
    def train_loop(self):
        ...
        while rclpy.ok():
            ...
            for t in range(MAX_STEPS):
                ...
                self.action_pub.publish(msg); self.sim_sleep(DT)
...
def main(): rclpy.init(); rclpy.spin(HumanoidServoNodeV2())
