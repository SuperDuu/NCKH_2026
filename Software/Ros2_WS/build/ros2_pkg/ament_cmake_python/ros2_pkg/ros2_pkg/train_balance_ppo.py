...
class ActorCritic(nn.Module):
    def __init__(self, state_dim, action_dim, action_std_init=0.1060):
        super(ActorCritic, self).__init__()
        # Khởi tạo biến action_var để lưu STD
    ...
    def set_action_std(self, new_action_std):
        # Cập nhật lại giá trị phương sai (variance) dựa trên STD mới
        ...
        while rclpy.ok():
            ...
            if not failed_at_start:
                ...
                if ep % 100 == 0 and ep > 0:
                    # Tính toán STD mới (giảm 2%)
                    current_std = torch.sqrt(self.ppo.policy.action_var[0]).item()
                    new_std = max(0.1, current_std * 0.98) 
                    
                    # Cập nhật cho cả hai mạng
