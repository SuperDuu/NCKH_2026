...
import json
import csv
import shutil # Để copy file weights
from datetime import datetime

# ==============================================================================
# CẤU HÌNH VẬT LÝ & THỜI GIAN (GIỮ NGUYÊN TUYỆT ĐỐI LOGIC CŨ)
# ==============================================================================
...
LIFT_H = 0.03 
ACTION_DIM = 4 
STATE_DIM = 7 # TĂNG LÊN 7 ĐỂ NHẬN THÊM GÓC YAW
STEPS_PER_PHASE = 15 
...
class ActorCritic(nn.Module):
    ...
    def set_action_std(self, new_action_std):
        action_var = torch.full((self.actor[-2].out_features,), new_action_std * new_action_std).to(DEVICE)
...
class PPO:
    def __init__(self, state_dim, action_dim):
        self.gamma = 0.99
        self.eps_clip = 0.2
        self.K_epochs = 20 # TĂNG LÊN 20 THEO YÊU CẦU ĐỂ HỌC KỸ HƠN
        self.buffer_states, self.buffer_actions, self.buffer_logprobs = [], [], []
        self.buffer_rewards, self.buffer_is_terminals = [], []
        self.policy = ActorCritic(state_dim, action_dim).to(DEVICE)
        
        # GIẢM LEARNING RATE ĐỂ FINE-TUNING NHẸ NHÀNG
        self.optimizer = torch.optim.Adam([
            {'params': self.policy.actor.parameters(), 'lr': 0.00003}, # 3e-5
            {'params': self.policy.critic.parameters(), 'lr': 0.0001}  # 1e-4
        ])
...
# ==============================================================================
# ROS2 TRAINING NODE (VỚI QUẢN LÝ THƯ MỤC FINE-TUNING)
# ==============================================================================
class HumanoidTrainNode(Node):
    def __init__(self):
        ...
        self.old_best_path = os.path.join(self.base_dir, "best_model.pth")
        self.best_model_path = os.path.join(self.ft_dir, "best_model.pth")
        self.latest_model_path = os.path.join(self.ft_dir, "latest_model.pth")
        self.meta_path = os.path.join(self.ft_dir, "training_meta.json")
        self.history_path = os.path.join(self.ft_dir, "training_history_yaw.csv")

        # LOGIC COPY FILE BEST CŨ NẾU CHƯA CÓ TRONG THƯ MỤC FINE-TUNING
        if not os.path.exists(self.best_model_path) and os.path.exists(self.old_best_path):
            shutil.copy(self.old_best_path, self.best_model_path)
            shutil.copy(self.old_best_path, self.latest_model_path)
            print("\033[94m>>> Copied old best_model.pth to finetunning_yaw directory <<<\033[0m")
    ...
    def load_training_state(self):
        ...
        # Load từ thư mục fine-tuning
        load_path = self.latest_model_path if os.path.exists(self.latest_model_path) else self.best_model_path
        if os.path.exists(load_path):
            # Xử lý size mismatch do tăng STATE_DIM (từ 6 lên 7) bằng cách chỉ load các lớp tương thích
            pretrained_dict = torch.load(load_path, map_location=DEVICE)
            model_dict = self.ppo.policy.state_dict()
            # Lọc bỏ lớp đầu tiên của Actor và Critic vì STATE_DIM thay đổi
            filtered_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict and v.size() == model_dict[k].size()}
            model_dict.update(filtered_dict)
            self.ppo.policy.load_state_dict(model_dict)
            self.ppo.policy_old.load_state_dict(self.ppo.policy.state_dict())
            print(f"\033[92m>>> Loaded Weights (Partial for State 7) from {load_path} <<<\033[0m")
    ...
    def check_safety_and_prepare_cmd(self, action):
        ...
        if weight_shift > 0: 
            target_y_l = 0.01 - 0.012 * weight_shift + dy_s 
            target_y_r = -0.01 - dst_s 
        elif weight_shift < 0: 
            target_y_r = -0.01 + 0.012 * abs(weight_shift) - dy_s 
            target_y_l = 0.01 + dst_s 
    ...
    def reset_simulation(self): 
         ...
         self.reset_pub.publish(Bool(data=False)); self.roll = 0.0; self.pitch = 0.0; self.yaw = 0.0; self.is_left_support = True; self.step_in_episode = 0
    ...
    def train_loop(self):
        ...
        while rclpy.ok():
            ...
            for t in range(MAX_STEPS):
                ...
                # REWARD FINE-TUNING YAW (SIÊU CÂN BẰNG)
                r_alive = 1.5
                r_balance = 4.5 * np.exp(-12.0 * np.sqrt(self.smooth_pitch**2 + self.smooth_roll**2))
                r_yaw = -3.0 * abs(self.yaw) # PHẠT XOAY YAW
                r_energy = -0.4 * np.sum(np.square(action)) # PHẠT XOẠC CHÂN/ENERGY
                
                reward = r_alive + r_balance + r_yaw + r_energy + pen
            ...
            if not failed_at_start:
                ...
                print(f"FT-Ep {ep} | Reward: {ep_reward:.2f} | Steps: {t} | Yaw: {self.yaw:.3f} | {reason}")
                ep += 1
                ...
                if ep % 100 == 0 and ep > 0:
                    ...
                    self.ppo.policy_old.set_action_std(new_std)
