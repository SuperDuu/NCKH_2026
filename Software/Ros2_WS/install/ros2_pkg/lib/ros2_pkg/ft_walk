...
# ==============================================================================
# CẤU HÌNH TỐI ƯU CHO ROBOT 30CM & SERVO MG996R
# ==============================================================================
...
STD_Z, STD_Y = 0.195, 0.01
LIFT_H = 0.035  # Hạ thấp độ nhấc chân để phù hợp robot 30cm
...
# ==============================================================================
# MẠNG NEURAL (PPO) - FIX LỖI ATTRIBUTEERROR & BỔ SUNG LOG UPDATE
# ==============================================================================
class ActorCritic(nn.Module):
    ...
    def set_action_std(self, new_action_std):
        # FIX: Trỏ trực tiếp vào ACTION_DIM thay vì truy cập lớp Tanh
...
class PPO:
    ...
    def update(self):
        ...
        print(f"\n\033[1;32m[PPO UPDATE] >>> Đang huấn luyện với {min_size} steps...\033[0m")
        ...
        print("\033[1;32m[PPO UPDATE] >>> Thành công. Trọng số mới đã được áp dụng.\033[0m\n")
...
class HumanoidWalkNode(Node):
    def __init__(self):
        ...
        # MỤC TIÊU VẬN TỐC THỰC TẾ CHO ROBOT 30CM 
        self.target_v_x = 0.05 
        ...
        print(f"\033[94m>>> Node Ready. WALK Fine-tuning. Target: {self.target_v_x} m/s\033[0m")
    ...
    def check_safety_and_prepare_cmd(self, action):
        ...
        dx_s, dy_s, dz_s, dst_s = smooth_act[0]*0.08, smooth_act[1]*0.02, smooth_act[2]*0.02, smooth_act[3]*0.02
        tz = STD_Z + dz_s
    ...
    def train_loop(self):
        ...
        while rclpy.ok():
            ...
            for t in range(MAX_STEPS):
                ...
                if not ok: 
                    self.ppo.buffer_rewards.append(-50.0); self.ppo.buffer_is_terminals.append(True); break
                self.action_pub.publish(msg); time.sleep(DT)
                
                # --- REWARD ĐI BỘ THỰC TẾ ---
                done = abs(self.smooth_roll) > 0.65 or abs(self.smooth_pitch) > 0.65
                
                if self.vx > 0.01: # Robot bắt đầu nhích
                    r_alive = 4.0
                    r_balance = 8.0 * np.exp(-12.0 * np.sqrt(self.smooth_pitch**2 + self.smooth_roll**2))
                    r_vel = 12.0 * np.exp(-5.0 * abs(self.vx - self.target_v_x))
                else:
                    r_alive = 1.0 # Ép di chuyển mới có thưởng sống sót cao [cite: 25]
                    r_balance = 3.0 * np.exp(-10.0 * np.sqrt(self.smooth_pitch**2 + self.smooth_roll**2))
                    r_vel = -2.0 # Giảm mức phạt đứng im để tránh sụp đổ sớm [cite: 38]
                
                r_yaw = -2.5 * max(0.0, abs(self.yaw) - 0.15) 
            ...
            if not failed_at_start:
                ...
                ep += 1
            else: print("\033[91m⚠️ Reset Lag Detected. Skipping Episode.\033[0m")
