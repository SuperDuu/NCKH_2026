...
HISTORY_LEN = 7
FRAME_DIM = 10                  # [qw,qx,qy,qz, wx_norm,wy_norm,wz_norm, prev_dx,prev_dy,prev_dz]
STATE_DIM = HISTORY_LEN * FRAME_DIM  # 70
...
# --- Action scaling (ĐÃ GIẢM để tránh IK reject ở cực trị) ---
# Worst-case: sqrt(0.04² + 0.03² + 0.285²) ≈ 0.290 < SAFE_LIMIT 0.2927 ✓
SCALE_DX = 0.025                # ±2.5cm (giảm từ 4cm để giảm phản lực xoay)
SCALE_DY = 0.02                 # ±2cm (giảm từ 3cm)
SCALE_DZ = 0.02                 # ±2cm
...
PUSH_BASE_FORCE = 350.0
PUSH_MAX_FORCE = 500.0
...
# =============================================================================
# HISTORY BUFFER: 7 bước × 10D = 70D Observation
# =============================================================================
class HistoryBuffer:
    """
    Buffer lịch sử 7 bước, mỗi bước 10 chiều.
...
class PPOBalanceNode(Node):
    def __init__(self):
        ...
        # --- Joint publishers (reset) ---
        self.joint_pubs = {
            j: self.create_publisher(Float64, f'/model/humanoid_robot/joint/{j}_joint/cmd_pos', 10)
            for j in [
                'base_hip_left', 'hip_hip_left', 'hip_knee_left',
                'knee_ankle_left', 'ankle_ankle_left',
                'base_hip_right', 'hip_hip_right', 'hip_knee_right',
                'knee_ankle_right', 'ankle_ankle_right',
                'base_hip_middle', 'hip_shoulder_left', 'shoulder_shoulder_left',
                'shoulder_elbow_left', 'hip_shoulder_right', 'shoulder_shoulder_right',
                'shoulder_elbow_right']}
    ...
    def get_observation(self):
        ...
        frame = np.concatenate([
            ...
            self.smoothed_action.copy() # 3D: [prev_dx, prev_dy, prev_dz] ∈ [-1, 1]
        ])
    ...
    def compute_reward(self, action):
        ...
        # R_balance: Gaussian — gradient mạnh gần gốc, thúc đẩy chính xác cao
        r_balance = math.exp(-10.0 * (pitch * pitch + roll * roll))

        # R_jitter: -1.5 (giảm từ -5.0) — cân bằng với R_balance max=1.0
        # Khi Δa=0.3 trên 3 chiều: -1.5 * 3 * 0.09 = -0.405 < R_balance=1.0 ✓
        action_diff = action - self.prev_raw_action
        r_jitter = -1.5 * float(np.sum(action_diff ** 2))

        # R_yaw: phạt vận tốc góc quanh trục Z (gyro[2] = wz) VÀ phạt độ lệch yaw
        wz = float(self.gyro[2])
        r_yaw = -3.0 * wz * wz - 2.0 * abs(yaw)

        # R_restore: thưởng đứng yên (curriculum tăng dần)
        omega_sq = float(np.sum(self.gyro ** 2))
        action_sq = float(np.sum(action ** 2))
        r_restore = math.exp(-1.0 * omega_sq) * math.exp(-2.0 * action_sq)

        total_reward = r_balance + r_jitter + r_yaw + self.w_restore * r_restore
    ...
    def check_safety_and_prepare_cmd(self, raw_action):
        ...
        len_r = np.linalg.norm([tx_r, target_y_r, tz])
        if len_l > SAFE_LIMIT or len_r > SAFE_LIMIT:
            return False, None, -5.0

        msg = Float64MultiArray()
        msg.data = [tx_l, target_y_l, tz, lift_l,
                    tx_r, target_y_r, tz, lift_r, MOVE_DURATION]
        return True, msg, tz
    ...
    def is_fallen(self):
        ...
        return abs(roll) > FALL_ROLL_THRESHOLD or abs(pitch) > FALL_PITCH_THRESHOLD
    ...
    def reset_simulation(self):
        while not self.w_cli.wait_for_service(timeout_sec=1.0):
            pass
        # === RESET FLOW (giống ft_force.py — đã chứng minh ổn định) ===
        # 1. Báo LLC dừng nhận lệnh
        ...
        # 6. Chờ sim clock chạy lại (giống ft_force.py)
        sc = self.current_sim_time
        while self.current_sim_time - sc < 0.01:
            time.sleep(0.001)
        # 7. Báo LLC bắt đầu nhận lệnh lại
    ...
    def load_training_state(self):
        ...
        if os.path.exists(self.latest_model_path):
            self.ppo.policy.load_state_dict(
                torch.load(self.latest_model_path, map_location=DEVICE, weights_only=True))
            ...
            self.get_logger().info(
                f">>> Loaded: STD={self.current_std:.4f} best={self.best_reward:.2f} "
